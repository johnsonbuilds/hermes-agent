...
#
# Routing:
#   no args                       → exec `hermes gateway` (the fork default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `hermes <args>` (subcommand passthrough)
#
# Fork customization: the no-args default is `hermes gateway` (not bare
# `hermes`), preserving the pre-s6 `CMD [ "gateway" ]` contract from this
# gateway-focused fork (see upstream-sync-instructions.md).
#
...
if [ $# -eq 0 ]; then
    # Fork default: launch the gateway (mirrors pre-s6 `CMD [ "gateway" ]`).
    drop hermes gateway
fi

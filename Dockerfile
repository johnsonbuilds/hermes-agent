...
#
# This installs only the slim set of extras the gateway-focused fork
# image needs (messaging, cron, cli, pty, mcp, acp, dingtalk, feishu).
# We deliberately do NOT use the composite `[all]` extra or `--all-extras`:
# that would pull in heavy deps ([rl] = atroposlib + tinker + torch +
# wandb from git, [yc-bench], [termux-all], plus the rest of `[all]`),
...
RUN touch ./README.md
# Fork customization: deliberately slim extras (NOT `--extra all`, which pulls
# heavy [rl]/[yc-bench]/[termux-all] deps). See upstream-sync-instructions.md.
RUN uv sync --frozen --no-install-project \
    --extra messaging --extra cron --extra cli --extra pty \
    --extra mcp --extra acp --extra dingtalk --extra feishu
...
# Deps are already installed in the cached slim-extras layer above;
# `--no-deps` makes this a fast (~1s) egg-link creation with no
...
#
# Fork note: this image's main-wrapper.sh defaults the no-args case to
# `hermes gateway` (not bare `hermes`), preserving the fork's
# gateway-as-default-startup contract from the pre-s6 `CMD [ "gateway" ]`.
